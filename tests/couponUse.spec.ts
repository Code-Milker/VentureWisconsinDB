...
describe('POST /trpc/couponUse', () => {
  ...
  it('should successfully redeem a valid non listing coupon', async () => {
    ...
    const group = await prisma.groups.findFirst({ where: { groupName: mockGroups[0].groupName } });
    ...
    const couponForUser = await prisma.couponsForUser.create({ data: { couponId: coupon.id, userEmail: user!.email } })
