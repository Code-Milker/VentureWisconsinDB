...
describe('POST /trpc/couponUse', () => {
  ...
  it('should return 404 if the user does not exist', async () => {
    ...
    const coupon = await prisma.coupon.findFirst({ where: { 'groupName': 'd pub' } });
    const listing = await prisma.listing.findFirst({ where: { name: 'd pub' } });
  ...
  it('should return 400 if the coupon has already been used', async () => {
    ...
    expect(response.body.error.message).toBe('This coupon has already been used.');
  });
  ...
  it('should return 404 if the listing associated with the coupon does not exist', async () => {
    ...
    expect(response.status).toBe(404);
    expect(response.body.error.data.code).toBe('NOT_FOUND');
    expect(response.body.error.message).toBe('Listing not found for this coupon.');
  });
  ...
  it('should return 400 if the coupon is not valid for the associated listing', async () => {
    ...
    expect(response.status).toBe(400);
    expect(response.body.error.data.code).toBe('BAD_REQUEST');
    expect(response.body.error.message).toBe('This coupon is not valid for the associated listing.');
  });
